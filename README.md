Deep Search Pro — 从零开发构建指南
本文档面向从零手写本项目的开发者，说明各功能模块的推荐实现顺序、依赖关系与具体怎么做。环境与运行步骤见同目录下的 从零构建与运行指南.md。
...
flowchart TB
  ...
  MAIN --> API --> FE
二、推荐实现顺序总览
按可验证、可增量原则，建议分 12 个阶段 完成（每阶段结束都应能单独跑通一个小 demo）。
...
load_dotenv(find_dotenv()) 加载根目录 .env。
使用 langchain.chat_models.init_chat_model，model=os.getenv("LLM_QWEN_MAX")，model_provider="openai"。
模块级导出单例 model，供全局复用。
验证：
...
YAML 分两块：main_agent.system_prompt、sub_agents 下三个子智能体的 name / description / system_prompt。
prompts.py 用 yaml.safe_load 在模块导入时加载一次，导出 main_agent_content、sub_agents_content。
提示词要写明：总指挥用 task 工具调度子智能体（subagent_type 为配置里的英文 key）。
...
建库 asset_vuln_db（或在脚本外 CREATE DATABASE）。
表 company_assets（资产）、asset_vulnerabilities（漏洞）。
插入若干未修复高危且互联网暴露的仿真数据，便于演示「暴露面 + CVE」场景。
...
子智能体在 DeepAgents 里不是类，而是一个 dict 配置：
...
申请 Tavily API Key，写入 .env 的 TAVILY_API_KEY。
...
9.1 基础设施
Docker 启动 Milvus（19530 端口）。
准备数据集：rawflow/ragdata/.../*.jsonl。
实现 rawflow/milvus_store.py：连接、fastembed 嵌入、similarity_search、集合统计。
实现 scripts/ingest_ragdata_to_milvus.py：分批写入，.env 中 INGEST_MAX_ROWS 可先设小值试跑。
...
创建 output/session_{session_id}/。
若存在 updated/session_{id}/，shutil.copy2 到 output 目录。
set_session_context + set_thread_context + monitor.report_session_dir。
拼接用户问题 + 【工作环境指令】（工作目录、上传文件列表、问答/研判规则）。
async for chunk in main_agent.astream(..., config={"configurable": {"thread_id": session_id}})。
解析 chunk：node_name == "model" 时，若有 tool_calls 且 name == "task" → monitor.report_assistant；若有 content → monitor.report_task_result。
...
先连 ws://127.0.0.1:8000/ws/demo-1。
再 POST /api/task，thread_id: demo-1。
WS 应收到 session_created、assistant_call、tool_start、task_result。
...
12.2 前端关键逻辑
会话 ID：threadId 输入框；空则生成 session-{timestamp}。
连接：ws://${location.host}/ws/${threadId}，更新状态 pill。
发任务：fetch('/api/task', { method:'POST', body: JSON.stringify({ query, thread_id }) })。
事件映射：tool_start / assistant_call / task_result / error → 不同样式的聊天气泡。
上传：FormData 带 thread_id，成功后启用「输出文件」按钮，GET /api/files?path=${sessionPath}。
...
十八、与运行指南的分工
文档	侧重
从零构建与运行指南.md	环境安装、.env、MySQL/Milvus 初始化、API 说明、排错
...
持久化会话：将 InMemorySaver 换为 Redis / Postgres checkpointer。
Linux PDF：用 WeasyPrint / md2pdf 替代 Word COM。
鉴权：API Key 或 JWT，WebSocket 握手校验。
子智能体并行：在总指挥提示词中允许「一次 task 描述多路并行」（取决于 DeepAgents 版本能力）。
评测集：固定 10 条 prompt + 期望调用的子智能体，做回归测试。
二十、核心代码索引
需求	文件
