...
# 将项目根目录加入 sys.path（直接 python api/server.py 时，默认只有 api/ 在 path 中）
...
app = FastAPI(title="Deep Search Pro · 企业安全智能助手")
...
async def index_page():
    ...
    if not index_path.is_file():
        return {"message": "index.html not found", "docs": "/docs"}
...
if __name__ == "__main__":
    # 在项目根目录启动，保证 uvicorn reload 能正确解析 api.server:app
    import os
